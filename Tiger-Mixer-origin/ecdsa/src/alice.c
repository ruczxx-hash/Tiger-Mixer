...
int payment_init(alice_state_t state, void *socket) {
  ...
  RLC_TRY {
    ...
    // Serialize the data for the message.
    bn_write_bin(payment_init_msg->data, RLC_BN_SIZE, state->sigma_hat_s->r);
...
int payment_done_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  bn_t q, x, sigma_s_inverse, gamma, tau_inverse;
  ec_t g_to_the_gamma;
  ...
  RLC_TRY {
    ...
    // 测量提取秘密值的时间（从签名中提取 gamma，然后去随机化得到 alpha_hat）
    START_TIMER(alice_extract_secret_alpha)
    ...
    END_TIMER(alice_extract_secret_alpha)
...
int main(void)
{
  ...
  long long program_start_time, program_end_time, program_total_time;

  alice_state_t state;
  alice_state_null(state);
  
  // 记录程序开始时间
  program_start_time = ttimer();
  ...
  // 记录程序结束时间并计算总耗时
  program_end_time = ttimer();
  program_total_time = program_end_time - program_start_time;
  printf("\n========================================\n");
  printf("Alice 总运行时间: %.5f sec\n", program_total_time / CLOCK_PRECISION);
  printf("========================================\n");

  // 输出时间测量表格
  output_timing_table("/home/zxx/A2L-origin/ecdsa/bin/alice_timing_results.csv");
  
  // 将总耗时添加到 CSV 文件
  FILE *fp = fopen("/home/zxx/A2L-origin/ecdsa/bin/alice_timing_results.csv", "a");
  if (fp) {
    fprintf(fp, "alice_total_time,%.2f\n", (program_total_time / CLOCK_PRECISION) * 1000.0);
    fclose(fp);
  }
