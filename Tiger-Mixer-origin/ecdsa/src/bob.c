...
int puzzle_solution_share_handler(bob_state_t state, void *socet, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    // 测量秘密提取时间（从 alpha_hat 和 beta 中提取 alpha）
    ...
    // 测量签名转换时间（使用 alpha 将预签名转换为最终签名）
    START_TIMER(bob_adaptor_transfer_sign_time)
    ...
    END_TIMER(bob_adaptor_transfer_sign_time)
...
int main(void)
{
  ...
  printf("\n========================================\n");
  printf("Bob 总运行时间: %.5f sec\n", program_total_time / CLOCK_PRECISION);
  printf("========================================\n");

  // 输出时间测量表格
  output_timing_table("/home/zxx/A2L-origin/ecdsa/bin/bob_timing_results.csv");
  
  // 将总耗时添加到 CSV 文件
  FILE *fp = fopen("/home/zxx/A2L-origin/ecdsa/bin/bob_timing_results.csv", "a");
  if (fp) {
    fprintf(fp, "bob_total_time,%.2f\n", (program_total_time / CLOCK_PRECISION) * 1000.0);
    fclose(fp);
  }
