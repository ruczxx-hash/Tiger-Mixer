...
general function \kbd{bnfinit} (which handles general number fields and gives
...
reduced using \kbd{qfbred}. To compose extended forms
$[q,d]$ and $[q,d']$, one may use \kbd{qfbcomp} and \kbd{qfbpow} but
ordinary multiplication and powerings also accept pairs.
...
\kbd{f}. Type
\bprog
 g = f; until(g == f, g = qfbred(g, 1); print(g))
@eprog\noindent
...
(A more user-friendly, but much less efficient, way to obtain the same result
is \kbd{algdep(a,2)} or \kbd{algdep(sa,2)}.)
...
\bprog
  ...
  bnfcertify(bnf);
@eprog\noindent
which is fast for small $D$ but costly in general. Indeed \kbd{bnfcertify}
needs the fundamental unit, which is large: we expect coefficients of about
$R/\log(10)\approx 1147$ digits, see \kbd{bnf.fu}! (We inserted the flag $1$
so that \kbd{bnfinit} would at least compute this fundamental unit in
factored form, which allows to reconstruct \kbd{bnf.fu}; \kbd{bnfcertify}
will often fail without it.) On the other hand, you can also try
\kbd{quadunit(D,'w)}. Impressive, isn't it? (You can check that its logarithm
is indeed equal to the regulator.)
...
the identity to \kbd{f} is around 4.253, given by the second component.
The distance function is approximately additive.
Since we want a total distance of $R$, this should be encountered
approximately at $R/f[2] \approx 621$ times the distance of \kbd{f}. Hence,
as a first try, we type \kbd{f \pow 621}. Oops, we overshoot: the distance is
now $3173.018\dots > R$. Now we can refine our initial estimate: we should be
close to the correct distance if we raise \kbd{f} to the power $621*R/3173$
which is a little less than $517$. Now if we compute \kbd{f\pow 517} we
hit the principal form right on the dot. This is not a lucky
accident: we must land extremely close to the correct target using this
method, and usually at most one reduction correction step is necessary.
And the distance component can tell us where we are along the cycle.
...
equivalent to a given form. (In fact, \kbd{f\pow 516} also yields the
principal form, so you can replace $517$ by $516$ if you wish!)

However, when PARI computes with extended forms, the specific order and
reductions that it performs are dictated entirely by the coefficients of the
quadratic form itself, and not by the distance component, hence the precision
used has no effect.

We now start again by setting the precision to (for example) 500 decimal
digits, we set \kbd{u = precision(u, 500)} (why is this necessary?), and then
\kbd{qfbred(u, 1)\pow 517} (or to the power $516$). We already know that we
land on the unit form, and the distance component gives us the regulator to
500 decimal places with no effort at all.
