...
{"_ZXQX_direct_compositum_worker",0,(void*)ZXQX_direct_compositum_worker,21,"GGGG","worker for ZX_direct_compositum"},
...
{"_ZX_direct_compositum_worker",0,(void*)ZX_direct_compositum_worker,21,"GGG","worker for ZX_direct_compositum"},
...
{"_parselect_worker",0,(void*)parselect_worker,21,"GG","_parselect_worker(d,C): evaluate the boolean closure C on d."},
...
{"ell2cover",0,(void*)ell2cover,12,"Gp","ell2cover(E): if E is an elliptic curve over Q, return a basis of the set of everywhere locally soluble 2-covers of the curve E. For each cover a pair [R,P] is returned where y^2-R(x) is a quartic curve and P belongs to E(k), where k = Q(x)[y] / (y^2-R(x))."},
...
{"galoissplittinginit",0,(void*)galoissplittinginit,10,"GDG","galoissplittinginit(P,{d}): Galois group over Q of the splitting field of P, that is the smallest field over which P is totally split. P is assumed to be integral, monic and irreducible; it can also be given by a nf structure. If d is given, it must be a multiple of the splitting field degree. The output is compatible with functions expecting a galoisinit structure."},
...
{"gcharduallog",0,(void*)gcharduallog,10,"GG","gcharduallog(gc,chi): return logarithm vector of character chi in R^n. We have gchareval(g,chi,x,0) = gcharduallog(g,chi)*gcharlog(g,x)"},
{"gchareval",0,(void*)gchareval,10,"GGGD1,L,","gchareval(gc,chi,x,{flag=1}): compute the evaluation chi(x) in C* if flag=1 and in C/Z if flag=0."},
...
{"gcharisalgebraic",0,(void*)gcharisalgebraic,10,"iGGD&","gcharisalgebraic(gc,chi,{&type}): returns 1 if chi is an algebraic (type A0) character. If type is present, set it to the infinity type [p,q] of chi."},
...
{"gcharnewprec",0,(void*)gcharnewprec,10,"Gp","gcharnewprec(gc): given a Grossencharacter group \\kbd{gc}, recompute its invariants to ensure accurate results to current precision."},
...
{"getcache",0,(void*)getcache,15,"","getcache(): returns information about various auto-growing caches. For each resource, we report its name, its size, the number of cache misses (since the last extension) and the largest cache miss."},
...
{"hyperelldisc",0,(void*)hyperelldisc,12,"G","hyperelldisc(X): X being a nonsingular hyperelliptic model of a curve, return its discriminant. X can be given either by a squarefree polynomial P such that X:y^2=P(x) or by a vector [P,Q] such that X:y^2+Q(x)*y=P(x) and Q^2+4P is squarefree."},
...
{"lambertw",0,(void*)glambertW,8,"GD0,L,p","lambertw(y,{branch=0}): solution of the implicit equation x*exp(x)=y. In the p-adic case, gives a solution of x*exp(x)=y if x has positive valuation, of x+log(x)=log(y) otherwise."},
...
{"lfuneuler",0,(void*)lfuneuler,13,"GGp","lfuneuler(L,p): return the Euler factor at p  of the L-function given by L (Lmath, Ldata or Linit) if it is exists and can  be determined."},
...
{"mfshimura",0,(void*)mfshimura,15,"GGD1,L,","mfshimura(mf, F, {D = 1}): F being a modular form of half-integral weight k >= 3/2 and t a positive squarefree integer, computes the Shimura lift G of weight 2k-1 corresponding to D. This function returns [mf2,G,v], where mf2 is a modular form space containing G, and v the vector of coefficients of G on mf."},
...
{"mftocoset",0,(void*)mftocoset,15,"LGG","mftocoset(N,M,Lcosets): M being a matrix in SL_2(Z) and Lcosets being mfcosets(N), find the right coset of G_0(N) to which M belongs. The output is a pair [ga,i] such that M = ga * Lcosets[i], with ga in G_0(N)."},
...
{"nfeltispower",0,(void*)nfispower,10,"lGGLD&","nfeltispower(nf,x,n,{&y}): return 1 if x is an n-th power (and set y to an n-th root if present), else return 0."},
{"nfeltissquare",0,(void*)nfissquare,10,"lGGD&","nfeltissquare(nf,x,{&y}): return 1 if x is a square (and set y to a square root if present), else return 0."},
...
{"nfresolvent",0,(void*)nfresolvent,10,"GD0,L,","nfresolvent(pol,{flag=0}): In the case where the Galois closure of the number field defined by pol is S3, Dl, A4, S4, F5, A5, M21, or M42, give the corresponding resolvent field. Otherwise, give a \"canonical\" subfield, or if flag >= 2 all \"canonical\" subfields. If flag is odd, give also the \"conductor\" f, whose definition is specific to each group."},
...
{"plotcolor",0,(void*)plotcolor,17,"LG","plotcolor(w,c): in rectwindow w, set default color to c. Possible values for c are [R,G,B] values, a color name or an index in the graphcolormap default: factory settings are 1=black, 2=blue, 3=sienna, 4=red, 5=green, 6=grey, 7=gainsborough. Return [R,G,B] value attached to color."},
...
{"poltomonic",0,(void*)poltomonic,6,"GD&","poltomonic(T,{&L}): T in Q[x]; return U monic in Z[x] such that U(x) = C T(x/L) for some rational C and L. If &L is given, set it to L."},
...
{"qfbcornacchia",0,(void*)qfbcornacchia,5,"GG","qfbcornacchia(d,n): Solve the equation x^2+dy^2 = n in integers x and y where d > 0 and n is prime or 4 times a prime."},
...
{"qfbred",0,(void*)qfbred0,5,"GD0,L,DGDG","qfbred(x,{flag=0},{isd},{sd}): reduction of the binary quadratic form x. All other args. are optional. The argument isd and sd, if present, supply the values of floor(sqrt(d)) and sqrt(d) respectively, where d is the discriminant. If d<0, its value is not used and all references to Shanks's distance hereafter are meaningless. flag can be any of 0: default; 1: do a single reduction step;"},
{"qfbredsl2",0,(void*)qfbredsl2,5,"GDG","qfbredsl2(x,{isD}): reduction of the binary quadratic form x, return [y,g] where y is reduced and g in Sl(2,Z) is such that g.x = y; isD, if present, must be equal to sqrtint(D), where D > 0 is the discriminant of x."},
...
{"qfminimize",0,(void*)qfminimize,7,"G","qfminimize(G): given a square symmetric matrix G with rational  coefficients and non-zero determinant, of dimension n >= 1, return [H,U] such that H = c * U~*G*U for some rational c, and H integral with minimal determinant."},
...
{"quadunitindex",0,(void*)quadunitindex,5,"GG","quadunitindex(D, f): given a fundamental discriminant D, return the index of the unit group of the order of conductor f."},
{"quadunitnorm",0,(void*)quadunitnorm,5,"lG","quadunitnorm(D): return the norm of the fundamental unit of the quadratic order of discriminant D."},
...
{"serdiffdep",0,(void*)serdiffdep,6,"GLL","serdiffdep(s,p,r): find an inhomogenous linear differential equation satisfied by the series s, with polynomial coefficients of degree <= r. The result is a pair [E,P] such that E(d)(S)=P where E(d) is interpreted as a differential polynomial."},
...
{"setdebug",0,(void*)setdebug,1,"DsD-1,L,","setdebug({D},{n}): set debug level for domain D to n (n must be between 0 and 20). If n is omitted, return the current level for domain D. if D is omitted, return a two-column matrix which lists the available domains with their levels."},
...
{"snfrank",0,(void*)snfrank,7,"lGG","snfrank(D,q): assuming that D is a Smith normal form (i.e. vector of elementary divisors) for some module and q a power of an irreducible element or 0, return the rank of D/qD."},
...
{"subcyclohminus",0,(void*)subcyclohminus,10,"GDG","subcyclohminus(fH, {p=0}): Let F be the abelian number field contained in Q(zeta_f) corresponding to the subgroup H of (Z/fZ)^*. Let h be the relative class number h^-(F) and Q the Hasse unit index in {1,2}. If Q could be computed, return [h, Q]; else return [2h/Q, 0]."},
...
{"znsubgroupgenerators",0,(void*)znsubgroupgenerators,5,"GD0,L,","znsubgroupgenerators(H, {flag=0}): find generators of the subgroup H of (Z/fZ)^*; H is given by a vector of 1/0 values: the a-th component is 1 if and only if a belongs to H."},
