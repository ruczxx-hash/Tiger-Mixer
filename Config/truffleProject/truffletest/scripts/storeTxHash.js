...
module.exports = async function (callback) {
  try {
    const argv = require('minimist')(process.argv.slice(2), {
      string: ['contract', 'pool', 'id', 'txHash', 'from']
    });
    ...
    const txHash = String(argv.txHash || '');
    ...
    if (!isHex(txHash, 66)) throw new Error('invalid tx hash');
    ...
    console.log('[DEBUG] storeTxHash params:', {
      contractAddr,
      escrowId: toBytes32(escrowId),
      txHash: toBytes32(txHash),
      fromAddr
    });

    const inst = await SignatureEscrow.at(contractAddr);
    const tx = await inst.storeTxHash(toBytes32(escrowId), toBytes32(txHash), { from: fromAddr });
