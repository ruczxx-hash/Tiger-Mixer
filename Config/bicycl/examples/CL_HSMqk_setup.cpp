...
main (int argc, char *argv[])
{
  ...
  /* */
  BICYCL::CL_Params cl_params(compact_variant);
  if (q.sgn() != 0)
    std::cout << BICYCL::CL_HSMqk (q, k, seclevel, randgen, cl_params);
  else
    std::cout << BICYCL::CL_HSMqk (qsize, k, seclevel, randgen, cl_params);
