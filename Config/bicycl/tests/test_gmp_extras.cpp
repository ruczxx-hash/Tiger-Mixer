...
bool
test_wNAF(RandGen &randgen)
{
  Mpz n, c, t;
  bool ret = true;

  for (size_t nbits = 1; nbits < 1280; nbits+=50)
  {
    for (unsigned int w = 2; w <= 16; w++)
    {
      for (size_t niter = 0; niter < 10; niter++)
      {
        n = randgen.random_mpz_2exp(nbits);
        if (randgen.random_bool())
          n.neg();

        WNAF wnaf(n, w);

        /* check that wnaf corresponds to n */
        c = 0UL;
        ...
        /* check the conditions on the digits of wnaf */
        if (wnaf.size() == 0)
          ret &= n.sgn() == 0; /* n should be 0 when wnaf is empty */
        else /* wnaf not empty */
        {
          /* wnaf not empty => n != 0 */
          ret &= n.sgn() != 0;
          /* most significant digit should not be 0 */
          ret &= wnaf[wnaf.size()-1] != 0;
          for (size_t i = 0; i < wnaf.size(); i++)
          {
            if (wnaf[i] != 0)
            {
              /* w nonzero consecutive digits should not exist */
              for (size_t j = i+1; j < std::min(wnaf.size(), i+w); j++)
                ret &= wnaf[j] == 0;
              /* nonzero digit should be odd */
              ret &= (std::abs(wnaf[i]) % 2) == 1;
              /* nonzero digit should be < 2^(w-1) in absolute value */
              ret &= std::abs(wnaf[i]) < (1UL << (w-1));
            }
          }
        }
      }
...
bool
test_conversion_to_bytes (RandGen &randgen)
{
  bool ret = true;
  ...
  for (int n = 10; n < 500; n+=3)
  {
    for (int i = 0; i < 25; i++)
    {
      d = randgen.random_mpz_2exp(n*8UL);
      // Testing conversion Mpz --> bytes
      bytes = static_cast<std::vector<unsigned char>>(d);
      // Conversion bytes --> Mpz is tested in 'test_constructor_from_bytes', so
      // we assume it works
      dbis = Mpz(bytes);
...
main (int argc, char *argv[])
{
  ...
  RUN_TEST_AND_PRINT_RESULT_LINE (test_wNAF, randgen);
  ...
  RUN_TEST_AND_PRINT_RESULT_LINE (test_conversion_to_bytes, randgen);
