...
bool check (RandGen &randgen, size_t q_nbits, size_t k, size_t Delta_OK_nbits,
            size_t niter, const string &name)
{
  CL_HSMqk C (q_nbits, k, Delta_OK_nbits, randgen);
...
{
  std::string desc = std::string (" ") + std::to_string (seclevel.nbits()) + " ";
  /* With a random q as big as the security level */
  CL_HSMqk C (seclevel.nbits(), k, seclevel, randgen);
  OpenSSL::HashAlgo H (seclevel);
...
main (int argc, char *argv[])
{
  ...
  success &= check (randgen, SecLevel::_112, 1, 20);
  success &= check (randgen, SecLevel::_112, 2, 20);
  success &= check (randgen, SecLevel::_128, 1, 15);
  success &= check (randgen, SecLevel::_192, 1, 3);
