...
int cp_ecdsa_ver(const bn_t r, const bn_t s, const uint8_t *msg, size_t len,
	...
	// 添加简化的调试输出
	printf("\n[RELIC_ECDSA_DEBUG] ========== cp_ecdsa_ver 函数调用 ==========\n");
	printf("[RELIC_ECDSA_DEBUG] 传入参数:\n");
	printf("  - msg指针: %p\n", msg);
	printf("  - len: %zu 字节\n", len);
	printf("  - hash标志: %d\n", hash);
		...
		if (bn_sign(r) == RLC_POS && bn_sign(s) == RLC_POS &&
			...
			if (bn_cmp(r, n) == RLC_LT && bn_cmp(s, n) == RLC_LT) {
				...
				if (!hash) {
					md_map(h, msg, len);
					// 输出原始消息和哈希值用于调试
					printf("[RELIC_ECDSA] 原始消息长度: %zu 字节\n", len);
					printf("[RELIC_ECDSA] 原始消息 (前32字节): ");
					for (size_t i = 0; i < (len < 32 ? len : 32); i++) {
						printf("%02x", msg[i]);
					}
					printf("\n");
					printf("[RELIC_ECDSA] 消息哈希 (%d字节): ", RLC_MD_LEN);
