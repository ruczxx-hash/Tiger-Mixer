...
def getAccountTransactions():
    ...
    try:
        ...
        if not exist:
            with lock:
                new_user = User.BadUser(" ", prompt.experiences_arrested[user_count % 10],
                                   prompt.personalities[user_count % len(config.total_moneys)], data['id'], total_moneys[user_count % 10],
                                   user_count)
