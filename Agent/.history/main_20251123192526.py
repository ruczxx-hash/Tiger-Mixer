...
def judge_illegal():
    ...
    data = request.get_json(silent=True)
    print(data)
    if not data or 'record' or 'id' not in data:
        return jsonify(error="not enough parameters"), 400
    else:
        ...
        bad_user_sequence = []
        for i in badusers:
            bad_user_sequence.append({"id":i.user_id,"sequence":i.transa})
        
        user_id = data['id']
        for i in judgers:
            if i.id == user_id:
                if i.judge(transaction_list,):
        ...
        juders.append(new_judger)
        if new_judger.judge(transaction_list,)
        count = 0
        for i in result:
            if i:
                count = count + 1
