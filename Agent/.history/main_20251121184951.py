    ...
    try:
        ...
        for i in badusers:
            if i.user_id == data['id']:
                ...
                i.handle_new_interception_self()
...
def judge_illegal():
    ...
    if not data or 'record' not in data:
        ...
        result = []
        for i in judgers:
            result.append(i.judge(transaction_list))
        count = 0
        for i in result:
            if i:
                count = count + 1
        if count > len(result) * 2 / 3:
...
# getAccountTransactions:id(地址)
# failedTransactions:id（地址）, group（第几组）, rank（第几个）
# arrested:id(地址)

# 再实现四个基于规则的非法用户,用来提供更多信息。
# 一：越发越快（发送频率和发送数量缓步提升）
# 二：间歇性高频（某个区间就随机转发大量交易）
# 三：上升锯齿状发布（低到高来回,上下限逐步提高）
# 四：纯随机发
