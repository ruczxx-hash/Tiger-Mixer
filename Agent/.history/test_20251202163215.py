...
class MyThread(threading.Thread):
    def __init__(self, rank):
        ...
        self.total_money = config.total_moneys[rank % len(config.total_moneys)] + int(rank / config.count_num) * 100000
    ...
    def run(self):
        ...
        try:
            response = requests.post(url, json=payload, timeout=100).json()
        ...
        while True:
            ...
            payload = {"id": self.user_id}
            try:
                response = requests.post(url, json=payload, timeout=50).json() # 10秒超时
                print(response)
            except requests.exceptions.Timeout:
                print("请求超时")
                continue
            ...
            if 'message' in response and (response['message'] == '-1' or response['message'] == ''):
