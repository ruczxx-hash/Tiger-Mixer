...
def generate_bad_sequence(n, group_size=10, probability_of_zero_group=0.2, probability_of_zero_value=0.2):
    """
    生成一个包含 n 个元组的序列，每个元组的第一个数递增，第二个数为随机数
    ...
    for i in range(n):
        ...
        if (i // group_size) > 0 and random.random() < probability_of_zero_group:
            ...
            # 否则，根据给定的概率生成第二个数
            second_number = 0 if random.random() < probability_of_zero_value else random.randint(1, 10)
...
# 示例：生成一个包含30个元组的序列，每组10个元组，整组第二个数有20%的概率为0
seq = generate_sequence(30, group_size=10, probability_of_zero_group=0.2, probability_of_zero_value=0.2)
print(seq)
