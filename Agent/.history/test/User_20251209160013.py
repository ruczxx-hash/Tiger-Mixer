...
from tools import calculate_sum, extract_json_from_markdown, sort_and_count_groupby
import asyncio
import config
import json
...
class UserAgent(AgentBase):
    ...
    async def reply(self):
        ...
        prompt_user = "Now it's the " + str(self.current_step) + " step of stage " + str(self.current_stage) + ". There are still " + str(self.current_task) + "ETH tasks to be completed in this stage. Your transaction record in this stage is " + str(self.transaction_list_of_this_stage) + ". Please present your trading plan for the current step. Remember to use the tool 'calculate_sum' to ensure that at the end of the stage, the total number of completed tasks in all its steps is equal to the task volume of that stage."
        msg = Msg(
            name="user",
            content=prompt_user,
            role="user",
    ...
    def check_if_finished_stage(self, msg: Msg) -> bool:
        ...
        if self.current_step == 6:
            ...
            transactions_this_step = josn.loads(decision_dict['res'])
    ...
    def handle_new_intercepted(self):
        asyncio.run(self.update_plan())
    

asyncio.run(UserAgent("User",100,0).reply())