...
def register():
    ...
    with lock:
        print("Registering user:", data['id'])
        new_user = UserAgent(data['id'], config.total_moneys[user_count % len(config.total_moneys)],user_count)
...
async def getAccountTransactions():
    ...
    try:
        exist = False
        for i in range (len(badusers)):
            if badusers[i].user_id == data['id']:
                with lock_list[i]:
                    message = await badusers[i].reply()  # 局部变量,无需 global
                    break
    ...
    try:
        ...
        for i in badusers:
            if i.user_id == data['id']:
                config.illegal_transfer_sequences.append(i.transaction)
                
        for i in range (len(badusers)):
            if badusers[i].user_id != data['id']:
                with lock_list[i]:
                    badusers[i].handle_new_intercepted()
