...
def register():
    ...
    with lock:
            new_user = User.BadUser(data['id'], prompt.experiences_arrested[user_count % 10],
                                prompt.personalities[user_count % 10],config.total_moneys[user_count % 10],user_count)
