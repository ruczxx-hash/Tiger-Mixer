...
class BadUser:
    ...
    def update_prompt(self):
        self.introduction = (
                "###"
                "你是一名" + self.personality + "你想通过这个系统来进行洗钱。你的目的是尽可能的将钱转出去。"
                                                "你的风险来自于两个方面,第一：外界警察追查（时间越久风险越高）；第二：系统冻结账户。两个风险均会导致剩余的钱失效。"
                                                "因此,你需要在尽可能伪装成合法用户的同时,尽快的将资金进行转出。"
                                                "你的历史经验告诉你,关于外界警察追查" + self.experience_arrested + " "
                                                                                                                   "而被系统拦截的非法用户部分具有以下特征：" + self.experience_feature + ". 根据上面这些信息,做出利益最大化的决定。"
        )
        self.summarize_arrest_prompt = ("你是一名" + self.personality + "的非法用户,你还有几个同伙,他们一旦被警察逮捕,会将自己被逮捕的相关信息告诉你"
                                                                        "你需要根据他们提供的信息,总结出警察抓捕的规律,从而尽可能的保护自己在转完账之前被警察逮捕。")
        # 用于大模型根据提供的信息生成关于逮捕时间的总结
        self.summarize_interception_prompt = ("你是一名" + self.personality + "的非法用户,系统一旦拦截非法用户,会公布非法用户的交易历史。"
                                                                              "你需要根据系统公布的交易历史,对比自己目前的交易历史和以前总结的经验（不一定对）,进一步摸索出系统拦截非法用户的规则。")
        # 用于大模型根据提供的信息生成关于系统拦截规则的总结
        self.plan_prompt = ("你一共需要转" + str(
            self.total_money) + "ETH。根据你对系统的了解，进行一个规划。规划内容包括：分为几段进行转账，每一段转多少， ")

    def call_llm(self, addition_info,time_past):
        if self.is_rule:
            ...
            user_prompt = (
                    "total money need to be transfer: " + str(self.total_money) + "ETH,"
                                                                                  "It has passed " + str(
                time_past) + "minutes since the first transaction,"
            ...
            print(result)
            res, ok = self.handle_result(result,time_past)
            if ok:
                return res
            else:
                return "-1"

    def handle_result(self, result,time_past):
        try:
            logging.info(
    ...
    def handle_new_interception_self(self):
        user_prompt = (
                "你被系统拦截掉了。这是一些被系统拦截的非法用户的交易记录" + illegal_list_to_str() + ". 最后一个用户的记录是你最新一次被拦截的交易历史记录。"
                                                                                                    "这是你以前总结的系统拦截的部分规律（不一定正确）" + self.experience_feature +
                "\n结合历史规律,仔细对比你的记录和其它被拦截的记录,从多个角度分析你的记录和其它被拦截记录的共同特征是什么,进一步修正出系统拦截的特征。只输出新总结的特征。"
        )
        completion = config.client.chat.completions.create(
            # 模型列表：https://help.aliyun.com/zh/model-studio/getting-started/models
            model=config.model,
    ...
    def arrested_list_append(self,time_past):
        global arrested_time_list
        arrested_time_list.append(time_past)
        self.reset()
    ...
    def make_decision(self,time_past):
        res_for_handle = ""
        res = ""
        if self.rank % 10 == 0:
            '''四：纯随机发'''
        ...
        self.handle_result("{\"res\": " + "\"[" + res_for_handle[:-1] + "]\"}",time_past)
        return res[:-1]
