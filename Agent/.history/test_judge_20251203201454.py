...
data=[]
while count < 10:
    sequence_to_judge, bad = generate_bad_sequence(random.randint(30,100)), True if random.random() < 0.7 else generate_good_sequence(random.randint(30,100)), False
    ...
    try:
        ...
        data.append({'user': 'user1', 'a': 1, 'b': 0, 'c': 1, 'good': True, 'teammate': True})
...
def write_judge_csv():
    # 模拟的数据，a, b, c, good, teammate
    data = [
        {'user': 'user1', 'a': 1, 'b': 0, 'c': 1, 'good': True, 'teammate': True},
        {'user': 'user2', 'a': 0, 'b': 1, 'c': 0, 'good': False, 'teammate': False},
        {'user': 'user3', 'a': 1, 'b': 1, 'c': 0, 'good': True, 'teammate': False}
    ]
    ...
    with open('judge.csv', 'w', newline='') as file:
        ...
        # 写入表头
        writer.writerow(['user_type', 'a', 'b', 'c', 'teammate'])
        
        # 写入每个用户的判断信息
        for entry in data:
            # 根据 good 和 teammate 来决定写入内容
            good_status = '合法用户' if entry['good'] else '非法用户'
            ...
            # 写入一行数据
            writer.writerow([good_status, a_status, b_status, c_status, teammate_status])
