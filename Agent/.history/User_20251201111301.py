...
class BadUser:
    ...
    def update_prompt(self):
        self.introduction = (
                ...
                "You are a" + self.personality + "You want to launder money through this system. Your aim is to transfer as much money out as possible."
                                                ...
                                                "Some of the illegal users intercepted by the system have the following characteristics" + self.experience_feature + ", You should transfer money as quickly as possible while avoiding showing the same characteristics as these as much as possible to prevent being intercepted by the system. Based on the above information, make a decision that maximizes benefits。"
        )
    ...
    def handle_new_interception(self):
        if len(illegal_list) % config.summarize_num_intercepted == 0:
            ...
            result = completion.choices[0].message.content
            print(user_id+" new experience: " + result)
            self.experience_feature = result
