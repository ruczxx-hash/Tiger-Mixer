        ...
        if not exist:
            with lock:
                new_user = User.BadUser(" ", prompt.experiences_arrested[user_count % len(config.total_moneys)],
                                   prompt.personalities[user_count % len(config.total_moneys)], data['id'], configtotal_moneys[user_count % len(config.total_moneys)],
                                   user_count)
...
def register():
    ...
    with lock:
            new_user = User.BadUser('''prompt.experiences_feature[user_count % 10]'''" ", prompt.experiences_arrested[user_count % 10],
                                prompt.personalities[user_count % 10], data['id'], config.total_moneys[user_count % 10],
                                user_count)
            message = new_user.call_llm(addition_info)
            badusers.append(new_user)
            user_count += 1
