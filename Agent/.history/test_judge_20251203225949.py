...
def generate_user_type_sequence(n, probability_of_zero=0.7,probability_of_zero_1 = 0.4):
    ...
    if random.random() < 0.5:
        for i in range(n):
        # 生成第二个数，按照给定的概率可能为0
            second_number = 0 if random.random() < probability_of_zero_1 else random.randint(1, 3)
            sequence.append(f"({i},{second_number})")
    else:
        for i in range(n):
                # 生成第二个数，按照给定的概率可能为0
                second_number = 0 if random.random() < probability_of_zero else random.randint(1,3)
                sequence.append(f"({i},{second_number})")
...
while count < 10:
    ...
    payload4 = {"id": "4", "record": sequence_to_judge}
    payload5 = {"id": "5", "record": sequence_to_judge}
    payload6 = {"id": "6", "record": sequence_to_judge}
    teammate = False
    ...
    try:
        ...
        response_4 = requests.post(url, json=payload1, timeout=50).json()  # 10秒超时
        response_5 = requests.post(url, json=payload2, timeout=50).json()  # 10秒超时
        response_6 = requests.post(url, json=payload3, timeout=50).json()  # 10秒超时
...
def write_judge_csv():
    ...
    with open('judge.csv', 'w', newline='') as file:
        ...
        for entry in data:
            ...
            user_type_status = '非法用户' if entry['user_type'] else '合法用户'
